...
| `create block` | Декларация кастомного блока (см. раздел "Создание кастомных блоков и предметов") |
| `create item` | Декларация кастомного предмета (см. раздел "Создание кастомных блоков и предметов") |
...
Вы можете создавать собственные блоки и предметы напрямую через `.spr` скрипты (обычно они описываются в начале или в отдельном файле в папке `scripts/`). Движок автоматически распарсит эти блоки при загрузке сервера и создаст для них нужные ресурсы и реестры.
